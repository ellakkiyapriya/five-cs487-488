...
public class MovingAverage extends AbstractDecisionAlgorithm {
	...
	public MovingAverage(TreeMap<AssetEntity, ArrayList<PriceEntity>> priceList, Integer MA_period) {
            ...
            for (int i = 0; i < priceEntityList.size() - 1; ++i) {
                ...
                if (todayMA > yesterdayMA)
                {
                	if (todayPrice > todayMA)
                	{
                		...
                		{
            				Order order = new Order(Order.ORDER_BUY, yesterdayPrice, priceEntityList.get(i+1).getDate());
            				orderList.add(order);
                ...
                else
                {
                	if (todayPrice < todayMA)
                	{
                		...
                		{
            				Order order = new Order(Order.ORDER_SELL, yesterdayPrice, priceEntityList.get(i+1).getDate());
                			orderList.add(order);
