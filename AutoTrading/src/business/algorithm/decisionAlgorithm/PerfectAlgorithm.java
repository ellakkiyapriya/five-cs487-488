...
public class PerfectAlgorithm extends AbstractDecisionAlgorithm {
	...
	public PerfectAlgorithm(TreeMap<AssetEntity, ArrayList<PriceEntity>> priceList) {
	...
	public OutputForDecisionAlgorithm runAlgorithm() {
    	...
    	for (AssetEntity asset : assetSet)
    	{
    		...
    		for (int i = 1; i < priceEntityList.size() - 1; ++i)
    		{
    			...
    			if ((yesterdayPrice < todayPrice) && (tomorrowPrice < todayPrice))
    				peakIndexList.add(i);
    			else if ((yesterdayPrice > todayPrice) && (tomorrowPrice > todayPrice))
