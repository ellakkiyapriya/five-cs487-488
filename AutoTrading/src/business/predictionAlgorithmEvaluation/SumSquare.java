...
public class SumSquare extends PredictionCriteria {
	...
	public TreeMap<String, Double> evaluate() {
		...
		AssetEntity assetEntity = (AssetEntity) paramOfPreditionCriteria
				.get("Asset");
		@SuppressWarnings("unchecked")
		TreeMap<Date, Double> priceList = (TreeMap<Date, Double>) paramOfPreditionCriteria
				.get("PriceList");

		Date beginDate = new java.sql.Date(priceList.firstEntry().getKey()
				.getTime());
		Date endDate = new java.sql.Date(priceList.lastEntry().getKey()
				.getTime());
		ArrayList<PriceEntity> realPriceList = priceManager.getPriceInInterval(
				assetEntity.getAssetID(), beginDate, endDate);
		double sum = 0;
		double t;
